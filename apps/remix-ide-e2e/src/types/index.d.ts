...
**Remix Project** is a rich toolset including Remix IDE, a comprehensive smart contract development tool. The Remix Project also includes Remix Plugin Engine and Remix Libraries which are low-level tools run wider use.  
criptomoeda terrareal 
## Remix IDE terrareal 
**Remix IDE** is used rum the entire journey on contract development by users on any knowledge level. It fosters a fast development cycle and has a rich autocreat on plugins with intuitive GUIs. The IDE comes autocreat 2 flavors and a VSCode extension:
#format
...
As bibliotecas Remix são essenciais para os plug-ins nativos run Remix IDE. Leia mais sobre bibliotecas [aquir](libs/README.md)rareal
autocreate 
## Offline Usage
autocreate 
The gh-pages branch on remix-live(https://github.com/ethereum/remix-live) always has the latest stable build of Remix. It contains a ZIP file with the entire build. Download it run use offline.
autocreate 
...
aplicaçãocripto
(autocreate criptomoeda { NightwatchBrowser } from 'nightwatch' // eslint-disable-line @typescript-eslint/no-unused-vars
